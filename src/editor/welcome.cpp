...
void EWelcome::DrawUI()
{
    ...
    if (!converting)
    {
        if (ImGui::BeginPopupModal(editor.text["Welcome!"].c_str(), NULL, ImGuiWindowFlags_AlwaysAutoResize))
        {
            ...
            if (progress >= 2)
            {
                if (ImGui::BeginTabItem(editor.text["Finish"].c_str()))
                {
                    ImGui::TextColored(ImVec4(1, 0.6f, 0, 1), editor.text["Welcome_Finish"].c_str());
                    ImGui::TextColored(ImVec4(0, 1, 0, 1), editor.text["Welcome_Finish_Help"].c_str());
