...
public:
    ...
    std::optional<u24> ReadU24()
    {

        // Get endian.
        u24 val;
        std::optional<u8> tmp;
        BStreamEndian endian = this->endian;
        if (endian == BStreamEndian::Native) endian = SystemEndian();

        // Read bytes.
        if (endian == BStreamEndian::Little)
        {
            tmp = Read<u8>();
    ...
    static void SwapEndianInPlace(T& item, BStreamEndian endian)
    {
        ...
        else if constexpr (ExposesFields<T>())
